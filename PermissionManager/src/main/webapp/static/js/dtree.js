...
// Node object
function Node(id, pid, name, url,checked, cval, title, target, icon, iconOpen, open) {	//console.log(id, pid, name, url,checked, title, target, icon, iconOpen, open);
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;		this.useChecked = checked || false;	//console.log(useChecked);    this.cval = cval;    
	this.title = title;
	this.target = target;
	this.icon = icon;
...
dTree.prototype.add = function(id, pid, name, url,checked,cval, title, target, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, url,checked, cval,title, target, icon, iconOpen, open);
};
...
dTree.prototype.node = function(node, nodeId) {
	...
	}		 //添加上复选框    if (this.config.check == true) {				console.log(node.cval);						//'"value = "'+ node.cval             str += '<label class="demo--label">'                     +'<input type="checkbox" name="rolepers" value="'+node.cval+'"  class="demo--radio" id="c' + this.obj + node.id                    + '" onclick="javascript:' + this.obj + '.cc(' + node.id                    + ',' + node.pid + ')"';		        if (node.useChecked){                 str += 'checked="checked"';                             }	                   str +='/>' + '<span class="demo--checkbox demo--radioInput"></span>'  + '</label>'       } 
	if (node.url) {
